 ...
 * ============================================================================
 * $Author: liuhui $
 * $Id: agent.php 17063 2010-03-25 06:35:46Z liuhui $
*/
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('ur_here',      $_LANG['14_agent_list']);
    $smarty->assign('action_link',  array('text' => $_LANG['13_agent_add'], 'href'=>'agent.php?act=add'));
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('ur_here',          $_LANG['13_agent_add']);
    $smarty->assign('action_link',      array('text' => $_LANG['14_agent_list'], 'href'=>'agent.php?act=list'));
    $smarty->assign('form_action',      'insert');
    $smarty->assign('user',             $user);
    $smarty->assign('special_ranks',    get_rank_list(true));
    $smarty->assign('agents',			agent_list(false));
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    assign_query_info();
    $smarty->assign('ur_here',          $_LANG['agent_edit']);
    $smarty->assign('action_link',      array('text' => $_LANG['14_agent_list'], 'href'=>'agent.php?act=list&' . list_link_postfix()));
    $smarty->assign('user',             $user);
    $smarty->assign('form_action',      'update');
    $smarty->assign('special_ranks',    get_rank_list(true));
    $smarty->assign('agents',			agent_list($user));
    $smarty->display('agent_info.htm');
}
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if (!$users->edit_user(array('username'=>$username, 'password'=>$password, 'email'=>$email, 'gender'=>$sex, 'bday'=>$birthday, 'parent_id'=>$parent_id ), 1))
    {
 ...
 * @return array
 */
function agent_list($user)
{
	$sql = "SELECT user_id, user_name, parent_id ".
				" FROM " . $GLOBALS['ecs']->table('users') . " u " .
				" LEFT JOIN " . $GLOBALS['ecs']->table('user_rank') . " r ON u.user_rank=r.rank_id " . 
				" WHERE rank_name='业务员' "; 
	$result = $GLOBALS['db']->getAll($sql);
	$agents = array();
	foreach($result as $row) {
		if($user == false || check_agent_valid($row, $user['user_id']))
			$agents[$row['user_id']] = $row['user_name'];
	}
	return $agents;
}

/**
 *  检查该agent是否有资格成为user的parent，检查一下user是否为agent
 *  的祖先，不然成为一个环状的结构。
 *
 * @access  public
 * @param
 ...
 * @return array
 */
function check_agent_valid($agent, $user_id)
{
	if((int)$agent['user_id'] == $user_id)
		return false;

	while((int)$agent['parent_id'] != 0)
	{
		if((int)$agent['parent_id'] == (int)$user_id)
			return false;
		$sql = "SELECT user_id, parent_id FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id=".(int)$agent['parent_id'];
		$agent = $GLOBALS['db']->getAll($sql);
		$agent = $agent[0];
	}
	return true;
}
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time, rank_name ".
			" FROM " . $GLOBALS['ecs']->table('users') . " u " .
			" LEFT JOIN " . $GLOBALS['ecs']->table('user_rank') . " r ON u.user_rank=r.rank_id " . $ex_where .
