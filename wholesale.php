...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price')
{
    ...
    else
    {
		...
		$price = get_wholesale_price($goods_id);
        $shop_price  = $price['price'] * $price['quantity'];
...
if ($_REQUEST['act'] == 'view')
{
	...
	else
	{
		...
		$ws_price = get_wholesale_price($goods_id);
		$ws_price['price'] = price_format($ws_price['price']);
		$smarty->assign('ws_price',          $ws_price);                       // 商品的销售排名
	...
	$smarty->display('wholesale_view.dwt',      $cache_id);
}
...
function wholesale_list($size, $page, $where)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$row['ws_price'] = get_wholesale_price($row['goods_id']);
        $row['url'] = 'wholesale.php?id='.$row['goods_id'].'&act=view';
...
function get_wholesale_price($goods_id)
{
